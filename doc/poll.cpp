...
// Ä¬ÈÏµÄ»½ĞÑº¯Êı,poll/select ÉèÖÃµÄ»Øµ÷º¯Êı»áµ÷ÓÃ´Ëº¯Êı»½ĞÑ
// Ö±½Ó»½ĞÑµÈ´ı¶ÓÁĞÉÏµÄÏß³Ì,¼´½«Ïß³ÌÒÆµ½ÔËĞĞ¶ÓÁĞ(rq)
int default_wake_function(wait_queue_t *curr, unsigned mode, int wake_flags,
                          void *key)
{
    // Õâ¸öº¯Êı±È½Ï¸´ÔÓ, ÕâÀï¾Í²»¾ßÌå·ÖÎöÁË
...
// epoll ÎÄ¼şÏµÍ³³õÊ¼»¯, ÔÚÏµÍ³Æô¶¯Ê±»áµ÷ÓÃ
static int __init eventpoll_init(void)
{
	// ÏŞÖÆ¿ÉÌí¼Óµ½epollµÄ×î¶àµÄÃèÊö·ûÊıÁ¿
    struct sysinfo si;
...
struct nested_call_node {
	struct list_head llink;
	void *cookie;   // º¯ÊıÔËĞĞ±êÊ¶, ÈÎÎñ±êÖ¾
	void *ctx;      // ÔËĞĞ»·¾³±êÊ¶
};
struct nested_calls {
